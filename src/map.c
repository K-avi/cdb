...
errflag_t map_insert(s_map *map, s_key *key, s_value *value){
    ...
    if(bucket.key){
        ...
        }while(bucket.key && start_index != index);//you can overwrite tombstones

        if(start_index == index){//will be handled better once i'll add rehasinh / elements counters
            return ERR_MAPFULL;
        }
    }
    atomic_store(&map->buckets[index], new_bucket );//atomically stores the new value
